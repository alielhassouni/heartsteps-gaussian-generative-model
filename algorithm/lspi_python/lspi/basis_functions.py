...
class ExactBasis(BasisFunction):
    ...
    def size(self):
        ...
        int
            ...
            (Referred to as k in the paper).
        """
        return len(self._num_states)*self.__num_actions
    ...
    def evaluate(self, state, action):
        ...
        """
        if len(state) != np.sum(self._num_states)/2:
        ...
        for state_var, num_state_values in zip(state, self._num_states):
            ...
            #print(state_var)
            #print(num_state_values)
            if state_var > num_state_values:
                raise ValueError('state values must be <= corresponding '
                                 + 'num_states value.')

        phi = np.zeros(self.size())
        len(self._num_states) * self.__num_actions
        base = action * ((len(self._num_states)))

        for i in range(base,base+len(state)):
            phi[i] = state[i-base]
